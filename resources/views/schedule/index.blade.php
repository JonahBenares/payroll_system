                ...
                <div class="flex justify-between  pb-4 bg-white white:bg-gray-900">
                    ...
                    <div class="flex">
                        <div x-data="{ modelOpen: false }">
                            <a href="{{ route('schedules.create') }}"  class="flex items-center justify-center px-3 py-2 mx-2 space-x-2 text-sm tracking-wide text-white capitalize transition-colors duration-200 transform bg-indigo-500 rounded-2xl white:bg-indigo-600 white:hover:bg-indigo-700 white:focus:bg-indigo-700 hover:bg-indigo-600 focus:outline-none focus:bg-indigo-500 focus:ring focus:ring-indigo-300 focus:ring-opacity-50">
                    ...
                    <table class="w-full text-sm text-left text-gray-500 white:text-gray-400">
                        ...
                        @foreach($sched as $sc)
                            <tr class="bg-white border-b white:bg-gray-800 white:border-gray-700 hover:bg-gray-50 white:hover:bg-gray-600">
                                <th scope="row" class="py-3 px-6 font-medium text-gray-900 whitespace-nowrap white:text-white">{{ $sc->schedule_code }}</th>
                                <td class="py-3 px-6">{{ date('h:i A', strtotime($sc->time_in)) }}</td>
                                <td class="py-3 px-6">{{ date('h:i A', strtotime($sc->time_out)) }}</td>
                                <td class="py-3 px-6 flex justify-center" align="center">
                                    <a href="{{ route('schedules.edit',$sc->id) }}" class="" title="Update">
                                    ...
                                    <form method="post" action="{{route('schedules.destroy',$sc->id)}}" accept-charset="UTF-8" style="display:inline">
                                    {{ method_field('DELETE') }}
                                    {{ csrf_field() }}
