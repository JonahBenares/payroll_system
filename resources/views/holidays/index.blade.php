		...
		<div class="flex flex-col flex-wrap sm:flex-row ">
			<div class="p-4 relative h-full w-full text-center bg-white rounded-2xl shadow-lg white:bg-gray-800 white:border-gray-700">
				<div class="flex justify-between  pb-4 bg-white white:bg-gray-900">
					...
					<div class="flex">
						<a href="{{ route('holiday.create') }}"  class="flex items-center justify-center px-3 py-2 mx-2 space-x-2 text-sm tracking-wide text-white capitalize transition-colors duration-200 transform bg-indigo-500 rounded-2xl white:bg-indigo-600 white:hover:bg-indigo-700 white:focus:bg-indigo-700 hover:bg-indigo-600 focus:outline-none focus:bg-indigo-500 focus:ring focus:ring-indigo-300 focus:ring-opacity-50">
					...
					<table class="w-full text-sm text-left text-gray-500 white:text-gray-400">
						...
						<tbody>
							...
							<tr class="bg-white border-b white:bg-gray-800 white:border-gray-700 hover:bg-gray-50 white:hover:bg-gray-600">
								...
								<td class="py-3 px-6">{{ $h->calendar_year }}</td>
								<td class="py-3 px-6 ">{{ $h->holiday_rate }}</td>
								<td class="py-3 px-6 flex justify-center " align="center">
									<a href="{{ route('holiday.edit',$h->id) }}" class="" title="Update">
									...
									<form method="post" action="{{route('holiday.destroy',$h->id)}}" onsubmit="return confirm('Are you sure you want to delete this record?');">
                                        @method('delete')
                                        @csrf
