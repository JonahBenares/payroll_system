        ...
        <div class="flex flex-col flex-wrap sm:flex-row">
            <div class="p-4 relative h-full w-full text-center  bg-white rounded-2xl shadow-lg white:bg-gray-800 white:border-gray-700">
                <div class="flex justify-between  pb-4 bg-white white:bg-gray-900">
                    ...
                    <div class="flex">
                        <a href="{{ route('adjustmentrate.create') }}"  class="flex items-center justify-center px-2 py-1 mx-2 space-x-2 text-sm tracking-wide text-white capitalize transition-colors duration-200 transform bg-indigo-500 rounded-2xl white:bg-indigo-600 white:hover:bg-indigo-700 white:focus:bg-indigo-700 hover:bg-indigo-600 focus:outline-none focus:bg-indigo-500 focus:ring focus:ring-indigo-300 focus:ring-opacity-50">
                ...
                <div class="overflow-x-auto h-100 relative  sm:rounded-lg">
                    <table class="w-full text-sm text-left text-gray-500 white:text-gray-400">
                        ...
                        <tbody>
                            @foreach($adjustmentrate AS $ar)
                            <tr class="bg-white border-b white:bg-gray-800 white:border-gray-700 hover:bg-gray-50 white:hover:bg-gray-600">
                                <th scope="row" class="py-4 px-6 font-medium text-gray-900 whitespace-nowrap white:text-white">
                                    {{ $ar->description }}
                                </th>
                                <td class="py-4 px-6">
                                    {{ ($ar->deduction_type==1) ? 'Percentage' : 'Actual Amount'; }}
                                </td>
                                <td class="py-4 px-6">
                                    {{ ($ar->deduction_type==1) ? $ar->rate_amount * 100 ."%" : number_format($ar->rate_amount,2); }}
                                </td>
                                <td class="py-4 px-6 flex justify-center" align="center">
                                    <a href="{{ route('adjustmentrate.edit' , $ar->id) }}" class="" title="Update">
                                    ...
                                    <form method="post" action="{{route('adjustmentrate.destroy',$ar->id)}}" onsubmit="return confirm('Are you sure you want to delete this record?');">
                                        @method('delete')
                                        @csrf
