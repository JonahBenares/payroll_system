        ...
        <div class="flex flex-col flex-wrap sm:flex-row ">
            <div class="p-4 relative h-full w-full text-center bg-white rounded-2xl shadow-lg white:bg-gray-800 white:border-gray-700">
                <div class="flex justify-between  pb-4 bg-white white:bg-gray-900">
                    ...
                    <div class="flex">
                        <a href="{{ route('statutorybracket.create') }}"  class="flex items-center justify-center px-2 py-1 mx-2 space-x-2 text-sm tracking-wide text-white capitalize transition-colors duration-200 transform bg-indigo-500 rounded-2xl white:bg-indigo-600 white:hover:bg-indigo-700 white:focus:bg-indigo-700 hover:bg-indigo-600 focus:outline-none focus:bg-indigo-500 focus:ring focus:ring-indigo-300 focus:ring-opacity-50">
                    ...
                    <table class="w-full text-sm text-left text-gray-500 white:text-gray-400">
                        ...
                        @foreach($statutory as $stat)
                            <tr class="bg-white border-b white:bg-gray-800 white:border-gray-700 hover:bg-gray-50 white:hover:bg-gray-600">
                                ...
                                <td class="py-3 px-6">
                                {{ number_format($stat->salary_from,2) }}
                                </td>
                                <td class="py-3 px-6">
                                {{ number_format($stat->salary_to,2) }}
                                </td>
                                <td class="py-3 px-6">
                                {{ number_format($stat->deduction_amount,2) }}
                                </td>
                                <td class="py-3 px-6">
                                {{ $stat->as_of }}
                                </td>
                                <td class="py-3 px-6 justify-center flex" align="center">
                                    <a href="{{ route('statutorybracket.edit', $stat->id) }}" class="" title="Update">
                                    ...
                                    <form method="post" action="{{route('statutorybracket.destroy',$stat->id)}}" accept-charset="UTF-8" style="display:inline">
                                    {{ method_field('DELETE') }}
                                    {{ csrf_field() }}
