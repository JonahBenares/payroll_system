            ...
            <div class="w-7/12">
                            ...
                            <div class="flex justify-between ">
                                <div class="px-6 py-6">
                                    ...
                                    <p class="font-bold text-lg text-white uppercase leading-snone">
                                        {{$start_date}}  <span class="mx-2"> ● </span> {{$end_date}}
                                    </p>
                ...
                <div class="mb-2 mx-0 xl:mr-2">
                    <div class="flex justify-between space-x-2">
                        <a href="{{ route('leavefailure.index',['month' => $month,'year' => $year,'period' => $period]) }}" class="shadow-md rounded-lg w-full bg-gradient-to-r from-emerald-300 to-lime-300">
                                ...
                                <div class="flex justify-between">
                                    ...
                                    <div class="px-6 py-4">
                                        <p class="font-bold text-2xl text-lime-400">
                                            {{($unfiled_leave!=0) ? str_pad($unfiled_leave, 2, "0", STR_PAD_LEFT) : '0'}}
                                        </p>
                                    </div>
                                </div>
                            </div>
                        ...
                        <a href="{{ route('ot.index',['month' => $month,'year' => $year,'period' => $period]) }}" class="shadow-md rounded-lg w-full bg-gradient-to-r from-emerald-300 to-lime-300">
                            <div class="bg-[url('images/circle.png')] bg-[length:170px_100px] bg-no-repeat bg-right-top rounded-lg"> 
                                <div class="flex justify-between">
                                    <div class="px-6 py-4">
                                        <p class="text-sm text-white leading-none">
                        ...
                        <ul>
                            ...
                            <li class="flex items-center text-gray-600 white:text-gray-200 justify-between py-3 border-b-2 border-gray-100 white:border-gray-800 px-4 bg-red-100">
                                <table class="w-full text-sm border-0">
                                    <tr>
                                        <td align="center">Set Holiday for ({{date('Y',strtotime('+1 year'))}}) </td> 
                            ...
                            <li class="flex items-center text-gray-600 white:text-gray-200 justify-between py-3 border-b-2 border-gray-100 white:border-gray-800 px-4">
                                    ...
                                    <table class="w-full text-sm border-0">
                                        <tr>
                                            <td align="left" class="px-2 text-sm" width="20%">{{date('M. d,Y',strtotime($r->reminder_date))}}</td>
                                            <td align="left" class="hover:text-blue-500">
                                                <a data-modal-target="updateRem" data-modal-toggle="updateRem" id='reminderUpdate' data-id='{{$r->id}}' data-reminder='{{$r->reminder}}' data-date='{{$r->reminder_date}}' class="w-full cursor-pointer" title="Update Reminder">
                                                    {{$r->reminder}}
                                                </a>
                                            </td>
                                            <td align="right" class="" width="5%" >
                                                ...
                                                <a href="{{ route('dashupdate',$r->id) }}" class="">
                                                ...
                                                <form method="GET" action="{{route('dashdestroy',$r->id)}}" onsubmit="return confirm('Are you sure you want to delete this record?');">
                                                    @method('delete')
                                                    @csrf
                            ...
                            <li class="flex items-center my-6 space-x-2">
                                ...
                                <div class="flex flex-col">
                                    ...
                                    <span class="text-sm text-gray-400 white:text-gray-300 ml-2">
                                        {{date('F d,Y',strtotime($h->holiday_date))}} 
                                    </span>
                                </div>
                            </li>
                            @endforeach
    ...
    <div id="medium-modal" tabindex="-1" class="fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-modal md:h-full">
        <div class="relative w-full h-full max-w-lg md:h-auto">
            ...
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                ...
                <form action="{{ route('dashstore') }}" method="POST" >
                    @csrf
                    <!-- Modal body -->
    ...
    <div id="updateRem" tabindex="-1" class="fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-modal md:h-full">
        <div class="relative w-full h-full max-w-lg md:h-auto">
            ...
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                ...
                <form action="{{ route('dashupreminder') }}" method="POST" >
                    @csrf
                    <!-- Modal body -->
                    <div class="px-6 space-y-6">
