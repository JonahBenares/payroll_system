@if($queues && $showExporting)
    ...
    @if($batchExporting && !$batchFinished)
        <div wire:poll="updateExportProgress"
            ...
            <div class="white:text-slate-300">{{ trans('livewire-powergrid::datatable.export.exporting') }}</div>
            ...
            <div x-data={show:true} class="rounded-top">
                <div class="px-4 py-3 rounded-md cursor-pointer bg-slate-100 shadow white:bg-slate-600"
                    ...
                    <div class="flex justify-between">
                        ...
                        <x-livewire-powergrid::icons.chevron-double-down class="w-5 text-slate-400 white:text-slate-200"/>
                    </div>
                </div>
                <div x-show="show"
                    ...
                    @foreach($exportedFiles as $file)
                        <div class="flex w-full p-2">
                            <x-livewire-powergrid::icons.download
                                class="w-5 text-slate-700 white:text-slate-300 mr-3"/>
