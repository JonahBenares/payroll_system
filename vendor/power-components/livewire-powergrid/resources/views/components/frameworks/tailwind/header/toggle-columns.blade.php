@if(data_get($setUp, 'header.toggleColumns'))
    <div x-data="{open: false}"
        ...
        <button @click.prevent="open = ! open"
            ...
            <div class="flex">
                <x-livewire-powergrid::icons.eye-off class="text-slate-500 white:text-slate-300"/>
            </div>
        </button>
        ...
        <div x-show="open"
            ...
            @foreach($columns as $column)
                ...
                <div wire:click="$emit('pg:toggleColumn-{{ $tableName }}', '{{ $column->field }}')"
                     ...
                     class="@if($column->hidden) opacity-40 bg-slate-300 white:bg-slate-800 @endif cursor-pointer flex justify-start block px-4 py-2 text-slate-800 hover:bg-slate-50 hover:text-black-200 white:text-slate-200 white:hover:bg-gray-900 white:hover:bg-slate-700">
                    @if(!$column->hidden)
                        <x-livewire-powergrid::icons.eye class="text-slate-500 white:text-slate-300"/>
                    @else
                        <x-livewire-powergrid::icons.eye-off class="text-slate-500 white:text-slate-300"/>
                    @endif
                    <div class="ml-2">
