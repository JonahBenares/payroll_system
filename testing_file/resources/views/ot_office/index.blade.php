            ...
            <div class="p-4 relative h-full w-full text-center bg-white rounded-lg shadow-lg white:bg-gray-800 white:border-gray-700">
                ...
                <form action="{{ route('otOffice.store') }}" method="POST">
                    ...
                    <div class=" hover:overflow-x-auto overflow-x-hidden h-100 max-h-100 pt-2 pr-2 pl-2 mt-3 md:pt-0 md:pr-0 md:pl-0 ">
                                ...
                                @foreach($adjustment AS $adj)
                                    ...
                                    <tr class="appendsadj" id="appendsadj0">
                                        <td class="border border-gray-300 text-left">
                                            <div id='loader{{$x}}'>
                                                <input type="hidden" name="adjustment_det_id[]" id='adjustment_det_id{{$x}}' class="text-sm p-0 w-full border-0 px-2 adjustment_det_id" value="{{ (!empty($adj->id)) ? $adj->id : '0'}}">
                                            </div>
                                        ...
                                        <td class="border border-gray-300 px-2 text-right text-xs flex justify-between space-x-1 addmoreappendadj">
                                            <input type="text" name="total_amount[]" id="total_amount" class="text-xs p-0 w-full border-0 px-2 text-right total_amount" value="{{$adj->total_amount}}" onkeyup="calculateAdjustment()" {{ ($adj->amount=='0' && $adj->days_hr=='0' && $adj->total_amount!='0') ? 'style=background:#17f3f3;' : '' }}>
                                            <button class="text-sm bg-blue-600 text-white round-md p-1 addAdjustment">
                                                <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" viewBox="0 0 20 20" fill="currentColor">
                                                    <path fill-rule="evenodd" d="M10 3a1 1 0 011 1v5h5a1 1 0 110 2h-5v5a1 1 0 11-2 0v-5H4a1 1 0 110-2h5V4a1 1 0 011-1z" clip-rule="evenodd" />
                                                </svg>
                                            </button>
                                            <a id="delete_func{{$x}}" href="{{ route('destroyed',['id'=>$adj->id,'emp_id'=>$adj->employee_id]) }}" class="flex items-center justify-center px-1 py-1 text-sm tracking-wide text-white capitalize transition-colors duration-200 transform bg-red-500 white:bg-red-600 white:hover:bg-1red-700 white:focus:bg-red-700 hover:bg-red-600 focus:outline-none focus:bg-red-500 focus:ring focus:ring-red-300 focus:ring-opacity-50 delete_func" title="Delete" onclick="return confirm('Are you sure you want to delete this record?');">
                        ...
                        <input type="hidden" name="location_id" value="{{ (!empty($location_id)) ? $location_id : '' }}">
