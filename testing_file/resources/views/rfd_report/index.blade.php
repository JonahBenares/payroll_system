    ...
    <div class="overflow-auto h-screen pb-28 pt-2 pr-2 pl-2 md:pt-0 md:pr-0 md:pl-0">
        <div class="flex flex-col flex-wrap sm:flex-row ">
            <div class="p-4 relative h-full w-full text-center bg-white rounded-lg shadow-lg white:bg-gray-800 white:border-gray-700">
                ...
                <form class="items-center" method="POST" action="{{ route('rfdreport.update',$allowance_id) }}">
                    @csrf
                    @method('PUT')
                    @foreach($head AS $h)
                    <div class="flex justify-between space-x-40 w-full my-5">
                        <div class="space-y-1 w-8/12">
                            ...
                            <div class="flex justify-between space-x-2">
                                ...
                                <input name="pay_to"  type="text" class="px-0 text-sm bg-transparent border-0 text-left py-0 border-b border-gray-300 w-full" value="{{ getCompanyName($h->bu_id) . ' Employees as follows:' }}">
                            </div>
                            <div class="flex justify-between space-x-2">
                                <p class="text-sm text-sm w-40 text-left">Period Covered:</p>
                                <input type="text" class="px-0 text-sm bg-transparent border-0 text-left py-0 border-b border-gray-300 w-full" value="{{ date('M j, Y',strtotime($h->from_date)) .' to ' . date('M j, Y',strtotime($h->to_date)) }}">
                            </div>
                        </div>
                            ...
                            <tbody>
                                ...
                                <tr class="bg-white border-b white:bg-gray-800 white:border-gray-700 hover:bg-gray-50 white:hover:bg-gray-600">
                                    ...
                                    <td scope="row" class="py-2 px-2 font-medium text-gray-900 whitespace-nowrap white:text-white" align="center">
                                        <input type="text" class="px-0 w-full text-sm bg-transparent border-0 text-left" value="{{ $allowance_name }}">
                                    </td>
                                    <td scope="row" class="py-2 px-2 font-medium text-gray-900 whitespace-nowrap white:text-white" align="center">
                                        <input type="text" class="px-0 w-full text-sm bg-transparent border-0 text-left" value="{{ getEmployeeName($det->employee_id) }}">
                                    </td>
                                    <td scope="row" class="py-2 px-2 font-medium text-gray-900 whitespace-nowrap white:text-white" align="center">
                                        <input type="text" class="w-full text-sm bg-transparent border-0 text-center" value="{{ number_format($det->allowance_amount,2) }}">
                                    </td>
                                    <td scope="row" class="py-2 px-2 font-medium text-gray-900 whitespace-nowrap white:text-white" align="center">
                                        <input type="text" name="remarks[]" class="w-full text-sm bg-transparent border-0 text-left">
                                    </td>
                                    <td scope="row" class="py-2 px-2 font-medium text-gray-900 whitespace-nowrap white:text-white" align="center">
                                        <input type="text" class="w-full text-sm bg-transparent border-0 text-right" value="{{ number_format($det->total_allowance,2) }}">
                                        
                                    </td>
                                </tr>
                                <input type="hidden" name="allowance_detail_id[]" value="{{ $det->id }}">
                                @php $x++ @endphp
                                @endif
                                @endforeach
                                <tr class="bg-yellow-50 border-b white:bg-gray-800 white:border-gray-700 hover:bg-gray-50 white:hover:bg-gray-600">
                                    ...
                                    <td colspan="2" class="py-2 px-2 font-medium text-gray-900 whitespace-nowrap white:text-white" align="center">
                                        <div class="flex justify-between w-full space-x-5">
                                            ...
                                            <input type="text" class="bg-transparent border-0 w-full font-bold text-base bg-transparent border-0 text-right p-0" value="{{ number_format(array_sum($total),2) }}">
                                        </div>
                                    </td>
                                </tr>
                            </tbody>
