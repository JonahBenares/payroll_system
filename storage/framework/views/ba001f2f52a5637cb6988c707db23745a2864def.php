    ...
    <div class="overflow-auto h-screen pb-28 pt-2 pr-2 pl-2 md:pt-0 md:pr-0 md:pl-0">
        <div class="flex flex-col flex-wrap sm:flex-row ">
            <div class="p-4 relative h-full w-full text-center bg-white rounded-lg shadow-lg white:bg-gray-800 white:border-gray-700">
                ...
                <form action="<?php echo e(route('allowancerate.update',$ar->employee_id)); ?>" method='POST'>
                    <?php echo csrf_field(); ?>
                    <?php echo method_field('PUT'); ?>
                    
                    <div class="flex flex-row justify-between appends" id="appends0">
                        <div class="px-1 w-6/12">
                            <select name="allowance_name[]" id="allowance_name<?php echo e($x); ?>" class="text-sm w-full px-3 py-2 mt-2 text-gray-600 placeholder-gray-400 bg-white border border-gray-200 rounded-md focus:border-indigo-400 focus:outline-none focus:ring focus:ring-indigo-300 focus:ring-opacity-40 allowance_name" onchange='check_duplicate()'>
                                ...
                                <option value="<?php echo e($a->id); ?>" <?php echo e(($a->id==$ar->allowance_id) ? 'selected' : ''); ?>><?php echo e($a->allowance_name); ?></option>
                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                            </select>
                        </div>
                        <div class="px-1 w-5/12">
                            <input type="text" onkeypress="return isNumberKey(this, event)" name="allowance_rate[]" id="allowance_rate<?php echo e($x); ?>" class="text-sm w-full px-3 py-2 mt-2 text-gray-600 placeholder-gray-400 bg-white border border-gray-200 rounded-md focus:border-indigo-400 focus:outline-none focus:ring focus:ring-indigo-300 focus:ring-opacity-40 allowance_rate" value="<?php echo e($ar->allowance_rate); ?>">
                        </div>
                        <div class="px-1 w-1/12 addmoreappend flex justify-center space-x-1">
                            ...
                            <input type="hidden" name="allowance_rate_id[]" id="allowance_rate_id" value="<?php echo e($ar->id); ?>">
                            ...
                            <a id="delete_func<?php echo e($x); ?>" href="<?php echo e(route('destroy',['id'=>$ar->id,'emp_id'=>$ar->employee_id])); ?>" class="flex items-center justify-center px-2 py-2  my-2 space-x-2 text-sm tracking-wide text-white capitalize transition-colors duration-200 transform bg-red-500 rounded-2xl white:bg-red-600 white:hover:bg-red-700 white:focus:bg-red-700 hover:bg-red-600 focus:outline-none focus:bg-red-500 focus:ring focus:ring-red-300 focus:ring-opacity-50 delete_func" title="Delete" onclick="return confirm('Are you sure you want to delete this record?');">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4">
...
<?php endif; ?>
<?php /**PATH C:\xampp\htdocs\payroll_system\resources\views/all_rates/edit.blade.php ENDPATH**/ ?>